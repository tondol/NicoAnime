...
<div class="page-header">
	<h1><?= h($this->get_title()) ?> <small>クロール対象の登録</small></h1>
</div>
...
<?php
	...
	if (isset($this->post["confirm"])) {
		if ($is_valid) {
			$this->include_template("crawler/register_confirm.tpl");
		} else {
			$this->include_template("crawler/register_default.tpl");
		}
	} else if (isset($this->post["submit"])) {
		$this->include_template("crawler/register_submit.tpl");
	} else {
		$this->include_template("crawler/register_default.tpl");
	}
?>
